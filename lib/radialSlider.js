...
export function initRadialSlider(container, gsap, Draggable) {
  ...
  // Inserted behind the track (not appended after it) so the invisible
  // drag-capture circle doesn't sit on top of the cards and swallow clicks
  // on their links — items are already independent Draggable triggers, so
  // dragging by grabbing a card still works regardless of stacking order.
  collection.insertBefore(proxyWrap, collection.firstChild);
