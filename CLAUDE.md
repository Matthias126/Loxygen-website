...
- `brand-accent`: `#0EA5E9` — a light/sky blue used **only** for the emphasized clause inside page H1s (e.g. "The Academy — <span class="text-brand-accent">logistics and freight forwarding courses.</span>"). Previously a warm orange (`#FF6E03`), replaced because it no longer fit the brand; before that, an earlier navy-at-40%-opacity treatment read as a washed-out grey-blue. Do not use it for buttons, backgrounds, icons, or any other element — it's a single, deliberately narrow exception to the navy/white system, not a general-purpose third colour.
